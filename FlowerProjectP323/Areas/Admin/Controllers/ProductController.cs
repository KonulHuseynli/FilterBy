...
namespace Fiorello.Areas.Admin.Controllers
{
    ...
    public class ProductController : Controller
    {
        ...
        private IQueryable<Product> FilterProduct(ProductIndexViewModel model)
        {
            ...
            products = FilterByDate(products, model.CreatedAtStart, model.CreatedAtEnd);
            products = FilterByStatus(products, model.Status);
            return products;      
        }
        private IQueryable<Product> FilterByTitle(string title)
        {
          return  _appDbContext.Products.Where(p =>!string.IsNullOrEmpty(title) ? p.Name.Contains(title): true);
        }
        ...
        private IQueryable<Product> FilterByDate(IQueryable<Product> products, DateTime? createdatStar, DateTime? createdatEnd)
        {
            return products.Where(p => (createdatStar != null ? p.CreatedAt >= createdatStar : true) && (createdatEnd != null ? p.CreatedAt <= createdatEnd : true));
        }
